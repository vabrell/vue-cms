...
router.get('/products/visitedclicks', async (request, response, next) => {
  try {
    const db = await sqlite.open(process.env.DATABASE, { Promise }),
      // Get the top 3 products based on clicks
      products = await db.all('SELECT * FROM products order by visited desc limit 3')
...
router.get('/products/:id', async (request, response, next) => {
  ...
  try {
    ...
    if (!product) {
      ...
      // Update the database if someone visited a specific product	
      let visited = Number(product.visited)
      await db.run('UPDATE products SET visited=? WHERE id=?', [visited + 1, request.params.id])
...
router.post('/products', upload.single('file'), async (request, response, next) => {
  ...
  if (errors.length > 0) {
    ...
    try {
      ...
      if (request.file) {
        await db.run(
          'INSERT INTO products(name, description, image, price, stock, categories, visited) VALUES(?, ?, ?, ?, ?, ?, ?)',
          [
            request.body.name,
        ...
        await db.run(
          'INSERT INTO products(name, description, price, stock, categories, visited) VALUES(?, ?, ?, ?, ?, ?)',
          [
            request.body.name,
            request.body.description,
            request.body.price,
