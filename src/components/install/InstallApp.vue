<template>
  <b-container class="shadow-lg p-5 mt-5 rounded">
    ...
    <p v-if="step === 0">
      ...
      <strong>Starta installation</strong> för att
      starta.
    </p>
    ...
    <p v-if="step > 0 && step < 2" class="h5">
      ...
      <b-icon icon="check" variant="success" font-scale="1.5" v-if="!inProgress && step === 1" />
    </p>

    <b-form @submit.prevent="createUser" v-if="step > 1 && step < 3 && !inProgress">
    ...
    >Starta installation</b-button>

    <b-button variant="outline-primary" v-if="step === 1" @click="step++">Nästa</b-button>

    <b-button
      variant="success"
      v-if="step > 2"
      @click="installationCompleted"
    >Slutför installationen</b-button>
  </b-container>
</template>
